...
void SetListArduino::readSerial(int i){  // MODIFIED
    ...
    while (nSerial.available() > 0){
		...
		_char_passer[passer_tracker] = inChar;  // MODIFIED
        ...
        if (inChar == _serialTerm) {
            ...
            switch (*command) {
            	case _activateDeviceCmd:
            		...
            		#ifdef SETLIST_DEBUG
            			nSerial.print("Activating device: ");
            			nSerial.println(param);
            		#endif
            		...
            		if (param != NULL) {
						...
						if(channel >= 0 && channel < _deviceCount){
							...
							_deviceList[_activeDevice]->clearSetList();
								...
								nSerial.println("ArduinoError: Invalid Channel");
							#endif
						}
					} else {
						#ifdef SETLIST_ERROR_CHECK
							nSerial.println("ArduinoError: Invalid Param");
						#endif                  
					}      
            		break;
            	...
            	case _initRunCmd:
            		...
            		if (!_errorFlag){
						#ifdef SETLIST_DEBUG
							nSerial.println("Init SetList run...");
						#endif
            	...
            	case _echoSetListCmd:
            		...
            		#ifdef SETLIST_DEBUG
            			nSerial.println("Here is the programmed setlist:");
            		#endif
            		
            		for (int i = 0; i < _deviceCount; i++){
            			nSerial.print("Device #");
            			nSerial.println(i);
            			nSerial.print("Setlist lines: ");
            			int setlistLength = _deviceList[i]->getSetListLength();
            			nSerial.println(setlistLength);
            			for (int j = 0; j < setlistLength; j++){
            				nSerial.print("ln ");
            				nSerial.print(j);
            				nSerial.print("; Callback Ptr ");
            				nSerial.print(_deviceList[i]->getSetListFunc(j));
            				nSerial.print("; Params ");
            				int * lineParams = 
            					_deviceList[i]->getSetListParams(j);
            				for(int k = 0; k < MAX_PARAM_NUM; k++){
            					nSerial.print(lineParams[k]);
            					nSerial.print(" ");
            				}
            				nSerial.println(";");
            			}
            			...
            			if(setlistLength != _setlistLength){
            				nSerial.print("There is a mismatch in setlist lines. Device thinks there are ");
            				nSerial.print(setlistLength);
            				nSerial.print(" lines, while SetListImage thinks there are ");
            				nSerial.print(_setlistLength);
            				nSerial.println(" lines. Get yo' life together!");
            			}
            		}
            		break;
            	...
            	case _executeSingleLine:
            		...
            		#ifdef SETLIST_DEBUG
            			nSerial.println("Executing single line...");
            			nSerial.print("Channel: ");
            			nSerial.print(ch);
            			nSerial.print(" Line: ");
            			nSerial.println(ln);
            		#endif
            		if ( ch >= 0 && ch < _deviceCount){
            			...
            			#ifdef SETLIST_DEBUG
            				nSerial.print("Channel out of range. Only ");
            				nSerial.print(_deviceCount);
            				nSerial.println(" devices are registered.");
            			#endif
            		}
            		break;	
            }
            ...
            if (command != NULL && !specialCmd) {
                ...
                for (int i = 0; i < _commandCount; i++) {
                    #ifdef SETLIST_DEBUG
                    	nSerial.print("Trying to match command: ");
                    	nSerial.println(_commandList[i].command);
                    #endif
                    ...
                    if ((strncmp(command, _commandList[i].command, 
                        ...
                        #ifdef SETLIST_DEBUG
                        	nSerial.print("Matched command: ");
                        	nSerial.print(_commandList[i].command);
                        	nSerial.print(", Channel #: ");
                        	nSerial.println(_activeDevice);
							nSerial.println("These are the params: ");
                        #endif
                        ...
                        for (int p = 0; p < MAX_PARAM_NUM; p++){
                        	...
                        	if (paramChar != NULL){
                            	...
                            	#ifdef SETLIST_DEBUG
									nSerial.println(paramChar);
									nSerial.println(atoi(paramChar));
								#endif
                            } else {
                            	#ifdef SETLIST_DEBUG
									nSerial.print("param was null: ");
									nSerial.println(p);
                            	#endif
								paramList[p] = 0;
                            }
                        ...
                        #ifdef SETLIST_DEBUG
                        	nSerial.print("Parameters passed: ");
                        	for(int p = 0; p < MAX_PARAM_NUM; p++){
                        		nSerial.print(paramList[p]);
                        		nSerial.print(", ");
                        	}
                        	nSerial.println("");
                        	nSerial.print("Inserting into setlist line #: ");
                        	nSerial.println(_line);
                        #endif
                ...
                if (!matched) {
                    ...
                    #endif
                    #ifdef SETLIST_DEBUG
                    	nSerial.print("That was an invalid command. You sent ");
                    	nSerial.print("(");
                    	nSerial.print(_activeDevice);
                    	nSerial.print(",");
                    	nSerial.print(command);
                    	nSerial.println("), but valid commands (channel, cmd) are:");
                    	for (int i = 0; i < _commandCount; i++){
                    		nSerial.print("(");
                    		nSerial.print(_commandList[i].channel);
                    		nSerial.print(",");
                    		nSerial.print(_commandList[i].command);
                    		nSerial.print(")");
                    	}
			...
			if (_errorFlag){
				...
				if (run) {
				nSerial.println("GR");	// OK status to labview
				} else {
				nSerial.println("G");
				}
			}
