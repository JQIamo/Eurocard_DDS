...
//
void LCD_BASE::begin(uint8_t cols, uint8_t lines, uint8_t dotsize)
{
   if (lines > 1)
   {
...
void LCD_BASE::setCursor(uint8_t col, uint8_t row)
{
...
void LCD_BASE::noDisplay()
{
   _displaycontrol &= ~LCD_DISPLAYON;
   command(LCD_DISPLAYCONTROL | _displaycontrol);
}

void LCD_BASE::display()
{
   _displaycontrol |= LCD_DISPLAYON;
   command(LCD_DISPLAYCONTROL | _displaycontrol);
}
...
void LCD_BASE::noCursor()
{
   _displaycontrol &= ~LCD_CURSORON;
   command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void LCD_BASE::cursor()
{
   _displaycontrol |= LCD_CURSORON;
   command(LCD_DISPLAYCONTROL | _displaycontrol);
}
...
void LCD_BASE::scrollDisplayLeft(void)
{
   command(LCD_CURSORSHIFT | LCD_DISPLAYMOVE | LCD_MOVELEFT);
}

void LCD_BASE::scrollDisplayRight(void)
{
   command(LCD_CURSORSHIFT | LCD_DISPLAYMOVE | LCD_MOVERIGHT);
}

// This is for text that flows Left to Right
void LCD_BASE::leftToRight(void)
{
...
void LCD_BASE::rightToLeft(void)
{
   _displaymode &= ~LCD_ENTRYLEFT;
   command(LCD_ENTRYMODESET | _displaymode);
}
...
void LCD_BASE::noAutoscroll(void)
{
   _displaymode &= ~LCD_ENTRYSHIFTINCREMENT;
   command(LCD_ENTRYMODESET | _displaymode);
}
...
void LCD_BASE::createChar(uint8_t location, uint8_t charmap[])
{
...
#ifdef __AVR__
void LCD_BASE::createChar(uint8_t location, const char *charmap)
{
...
void LCD_BASE::noBacklight ( void )
{
   setBacklight(0);
}
...
// ---------------------------------------------------------------------------
void LCD_BASE::command(uint8_t value)
{
   send(value, COMMAND);
}

#if (ARDUINO <  100)
void LCD_BASE::write(uint8_t value)
{
   send(value, DATA);
}
#else
size_t LCD_BASE::write(uint8_t value)
{
   send(value, DATA);
   return 1;             // assume OK
}
